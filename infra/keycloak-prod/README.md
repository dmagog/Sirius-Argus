...
> Импорт срабатывает только при первом старте Keycloak (пустая БД). `deploy_netangels.sh` делает
> `down -v` при пересоздании — тогда импортируется этот realm. На уже инициализированной БД
> realm не переимпортируется автоматически.
