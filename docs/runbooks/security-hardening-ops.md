...
5. По-ролевое маскирование PII в UI (AUD-07) — ✅ **уже в коде** (11.06.2026): `auth.py:viewer_roles`
   берёт роль зрителя из `X-Auth-Request-Groups` (за oauth2-proxy) или Bearer (dev/тесты), не бросая
   401; маршрут `/data/dataset/{id}` считает `can_read_sensitivity(…, "pii")` и передаёт `unmasked`
   в `cards.dataset_card` — PII-сэмплы маскируются симметрично `/api/.../schema`. Этот шаг (3+4)
   лишь подаёт роль через forward_auth в проде; в dev маскирование уже работает по Bearer.
   Метаданные датасета (имя/источник/структура схемы) видны всем аутентифицированным ролям
   **by design** (visibility.read = все роли) — это каталог данных; клеарэнс-гейтятся только
   значения (PII-сэмплы), как и в API.

После этого закрываются AUD-03, AUD-07 (✅ маскирование уже сделано), AUD-29 (UI больше не анонимен и не самозаявлен).
