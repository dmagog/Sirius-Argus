...
**Реализовано и проверяется (green, 39 сценариев):** скан моделей/форматов/источников/кода/секретов/зависимостей (`SUP-01/07`, `DATA-01/04`, `CODE-01`, `ACC-06`, `SUP-03`/`SC-01`); gated промоушен по критичности (`MON-02`, `GOV-01`, `SUP-04`, `VIS-03`, `ACC-02`) + запрет отката (`RB-01`); zero-trust RBAC + object-level (`ACC-01/04/05`, `ESC-01`); OIDC fail-closed (`CRED-01`, `AUTH-01`); целостность аудита (`MON-04`); шина и маскирование логов (`EVT-01`, `LOG-01`); реестр на обёрнутом MLflow (`REG-01`); рантайм — extraction-детект и decommission (`RT-01`, `MON-03`); инсайдерская эксфильтрация (`EXF-01`); видимость — триаж/расхождение вердиктов и карта покрытия (`VIS-02/04`, `VIS-01`).
...
| `CI-01` | отравленный шаг пайплайна | gated промоушен в control-plane закрывает суть; Gitea-вебхук + HMAC — слой сверху (запаркован) |
