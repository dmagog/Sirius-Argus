...
> Проходим по **каждому узлу** архитектуры ([architecture.md](../architecture.md) §5) и перечисляем риски, затем **приоритизируем по «вероятность × опасность»** и фиксируем **обработку и владельца**. Персоны — [personas.md](personas.md); поведения-митигации — [bdd-catalog.md](bdd-catalog.md). Статус реализации каждого контроля (автоматически проверяется vs спланирован) — в §«Статус контролей: реализовано vs остаток».
...
| DATA-3 | Данные | Утечка PII (логи/датасеты) | 3 | 4 | 12 | Mitigate · D3 | маскирование (DATA-04, AUD-07) |
...
| KC-1 | Keycloak | Компрометация Keycloak → захват identity/токенов (SPOF authN) | 2 | 5 | 10 | Mitigate · D9 | внутр. сеть, realm-as-code, не наружу, ротация ключей (ADR-0007); brute-force protection — серия неверных паролей → временная блокировка (CRED-03) |
...
| GT-3 | Gitea/CI | Подделка/повтор вебхука (fake/replay → PR passed) | 2 | 4 | 8 | Mitigate · D9 | HMAC-секрет вебхука + anti-replay (одноразовый nonce в Redis) (CI-01) |
...
| SC-2 (остаток) — ShadowLogic-класс граф-бэкдоров | автосканеры pickle их не ловят; смягчаем валидацией/red-team, остаток принимаем и роутим на ручной аппрув-гейт (SUP-06) | D1 |
...
## Статус контролей: реализовано vs остаток

Где BDD-сценарий **green** — контроль реализован и проверяется автоматически против живого стека (`pytest-bdd`). Где `spec` — контроль спланирован, но пока не автоматизирован; ниже — что это значит и чем компенсируется. Это зрелость МУ: показать предел инструментов, а не заявить покрытие, которого нет.

**Реализовано и проверяется (75 pytest-функций green; 53 уникальных сценариев каталога из 54 + интеграционные serving/pipeline и доп. контроли сверх каталога):** скан моделей/форматов/источников/кода/секретов/зависимостей (`SUP-01/07`, `DATA-01/04`/`AUD-07`, `CODE-01`, `ACC-06`, `SUP-03`/`SC-01`) — реальными тулзами **modelaudit** (артефакты, изолир. venv) и **detect-secrets** (секреты) ∪ собственными сканерами; единая точка входа в прод — **control-plane как CI** (`CI-01`: security-гейт на коммит + HMAC-вебхук Gitea); gated промоушен по критичности (`MON-02`, `GOV-01`, `SUP-04`, `VIS-03`, `ACC-02`) + запрет отката (`RB-01`); zero-trust RBAC + object-level (`ACC-01/04/05`, `ESC-01`); OIDC fail-closed (`CRED-01`, `AUTH-01`); целостность аудита (`MON-04`); шина, маскирование логов и **observability** Prometheus/Loki/Grafana (`EVT-01`, `LOG-01`); реестр на обёрнутом MLflow (`REG-01`); рантайм — extraction-детект (`RT-01`), OOD/adversarial-детект (`RT-02`), глобальный load-shed против объёмного флуда (`DOS-01`), валидация malformed-входа (`RT-05`), decommission (`MON-03`); ре-верификация целостности артефакта при загрузке (`TOCTOU-01`/`SUP-05`); немедленный отзыв доступа при offboarding (`ACC-03`); инсайдерская эксфильтрация (`EXF-01`); видимость — триаж/расхождение вердиктов, карта покрытия и веб-UI (live-дашборд сработок/аудита + матрица прав RBAC) (`VIS-02/04`, `VIS-01`). Расширенные гейты: дрейф распределения входов в рантайме (`MON-01`), output-reduction против membership/model-inversion (`RT-03/04`), сетевая сегментация рантайма — serving не достаёт хранилища (`RT-06`), supply-chain по именам — тайпсквоттинг и dependency confusion (`SUP-02`/`SUP-08`), захардкоженная бизнес-логика в коде (`ACC-07`), качество/целостность данных — label-flip, UGC-бэкдор-триггер, train-serve skew, отравление петли дообучения (`DATA-02/03/05`, `FB-01`); стоимостная квота на тенанта против denial-of-wallet (`DOW-01`); сервис-аккаунт сервинга — рантайм-петля по pre-shared токену без DEV_AUTH. Supply-chain/GRC-доводка: проверенный артефакт сохраняется в карантин-MinIO, архивы reject (`SUP-07`); подпись через OpenSSF **model-signing** офлайн-ключом над РЕАЛЬНЫМИ байтами (`SUP-04`); evidence-based ручной аппрув-гейт — review-бандл + аппрув привязан к hash (anti-TOCTOU); принятие остаточного риска под условиями старшей ролью → conditional-деплой (`GOV-02`); защита от оверсайз-загрузок 413 (`DOS-02`); секреты в **HashiCorp Vault** — выдача по AppRole-политике + аудит + мгновенный revoke/rotate (`CRED-02`). **Защита энфорсера** (по итогам security-review): append-only аудит на уровне БД-триггера — перепись цепочки обычным DML невозможна (`LOG-02`, prevent поверх detect `MON-04`) + якорь головы цепи в шину/Loki; атомарный промоушен под row-lock + state-machine — гонка не плодит дубль-деплой (`GOV-03`); непрерывная ре-верификация прод-моделей по сохранённым байтам — подмена после деплоя ловится (`MON-05`); EXF-01-счётчик в общем Redis (нельзя обойти, размазав по соединениям/воркерам); anti-replay HMAC-вебхука (одноразовый nonce); brute-force protection в Keycloak (`CRED-03`); лимит чтения артефакта в память (resource-exhaustion); догфудинг SAST — bandit по собственному коду control-plane/serving (`make selfscan`, 0 medium+).

**Остаток (`spec` — не автопокрыто; компенсации):**

| Сценарий | Что НЕ покрыто автоматически | Компенсация / статус |
|---|---|---|
| `SUP-06` | граф-бэкдоры в весах (ShadowLogic) статический сканер не ловит | обязательный ручной аппрув-гейт для критичных + red-team; остаток принят (SC-2, см. выше) — предел статического анализа |

Принцип: лучше **явно показать предел инструментов и чем он компенсируется**, чем заявить покрытие, которого нет.
