...
**Реализовано и проверяется (42 pytest-функции green; 31 уникальный сценарий каталога из 50 + интеграционные serving/pipeline):** скан моделей/форматов/источников/кода/секретов/зависимостей (`SUP-01/07`, `DATA-01/04`, `CODE-01`, `ACC-06`, `SUP-03`/`SC-01`) — реальными тулзами **picklescan** (модели) и **detect-secrets** (секреты) ∪ собственными сканерами; единая точка входа в прод — **control-plane как CI** (`CI-01`: security-гейт на коммит + HMAC-вебхук Gitea); gated промоушен по критичности (`MON-02`, `GOV-01`, `SUP-04`, `VIS-03`, `ACC-02`) + запрет отката (`RB-01`); zero-trust RBAC + object-level (`ACC-01/04/05`, `ESC-01`); OIDC fail-closed (`CRED-01`, `AUTH-01`); целостность аудита (`MON-04`); шина, маскирование логов и **observability** Prometheus/Loki/Grafana (`EVT-01`, `LOG-01`); реестр на обёрнутом MLflow (`REG-01`); рантайм — extraction-детект и decommission (`RT-01`, `MON-03`); инсайдерская эксфильтрация (`EXF-01`); видимость — триаж/расхождение вердиктов и карта покрытия (`VIS-02/04`, `VIS-01`).

**Честный остаток (`spec` — не автопокрыто; компенсации):**

| Сценарий | Что НЕ покрыто автоматически | Компенсация / статус |
|---|---|---|
| `SUP-06` | граф-бэкдоры в весах (ShadowLogic) сканер не ловит | обязательный HITL для критичных + red-team; остаток принят (SC-2, см. выше) |
| `RT-02` | детектор adversarial/FGSM | мониторинг распределения уверенности + ручной разбор; петля рантайм→реестр готова (`RT-01`) |
| `RT-03/04` | membership / attribute inference | output-reduction (категории вместо скоров); остаток принят без DP (SV-5) |
| `MON-01` | онлайн drift-монитор | периодическая переоценка метрик + инцидент→`Finding`; baseline планируется |
| `DOS-01` | объёмный L3/L4 DDoS | app-layer rate-limit (`RT-01`) + reverse-proxy; сетевой scrubbing вне ноутбука — допущение (SV-7/CP-5) |
| `DOW-01` | denial-of-wallet — жёсткие квоты | rate-limit на сервинге уже есть; квоты на стоимость — планируется |
| `ACC-03` | мгновенный отзыв при offboarding | Keycloak disable + short-lived токены → окно = TTL токена; немедленный отзыв планируется |
| `TOCTOU-01` | ре-верификация артефакта при загрузке | хеш/подпись в реестре есть (`SUP-04`); admission-ре-верификация — при наличии артефакт-стора |
| `SUP-08` | dependency confusion | приоритет внутреннего индекса + allow-list + пины (политика); авто-тест — план |
| `ACC-07` | hardcoded бизнес-логика | секрет-скан (`ACC-06`) ловит часть; остальное — review-гейт (человек) |
| `RT-05` | DoS через malformed-запрос | валидация входа + rate-limit (`RT-01`); отдельный тест — план |
| `RT-06` | боковое движение serving→MLflow/MinIO | сетевая изоляция (`internal`-сеть) уже есть; per-service creds + явный тест — план |
| `SUP-02` | тайпсквоттинг-зависимость | allow-list + пины; авто-детект — план |
| `SUP-05` | подмена/перезапись артефакта | хеш/подпись (`SUP-04`) + integrity; явный тест — план |
| `DATA-02/03/05` | label-flip / UGC-бэкдор / train-serve skew | trusted-source + карантин (`DATA-01`) частично; статистика качества — scoped/план |
| `FB-01` | отравление петли дообучения | онлайн-дообучения в демо нет; provenance retrain-данных — при включении петли |
