...
| AUD-07 | ✅ | `production.yml`+oauth2-proxy убирает анонимный доступ к UI; **полный per-role unmask**: `auth.py:viewer_roles` (роль из `X-Auth-Request-Groups`/Bearer, без 401) → `dataset_card(unmasked=can_read_sensitivity(…,"pii"))` — та же политика, что в API; BDD «DS видит `***`, MLSecOps видит значение в UI» (data.feature) |
...
**✅ Сделано (11.06.2026):** `auth.py:viewer_roles(request)` — неблокирующая деривация роли зрителя (приоритет: `X-Auth-Request-Groups` за oauth2-proxy → Bearer той же валидацией, что `get_principal` → пусто; никогда не 401). Маршрут `/data/dataset/{id}` ([pages.py](../control-plane/app/routers/pages.py)) считает `unmasked = can_read_sensitivity(viewer_roles(request), "pii")` и передаёт в `cards.dataset_card(unmasked=…)`; та маскирует PII-сэмплы симметрично API `/api/datasets/{id}/schema` (дефолт — маска). Note в карточке стал честным (по факту допуска зрителя). Периметр (анон-доступ) по-прежнему закрывают oauth2-proxy/basic_auth (AUD-02/03) — здесь только «что показать». BDD: 2 новых сценария в `data.feature` (DS→`***`, MLSecOps→значение в server-rendered UI), прогон зелёный.
