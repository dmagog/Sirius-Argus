...
7. **Явные границы.** Остаточные риски (напр. ShadowLogic-класс бэкдоров) зафиксированы явно, с компенсацией.
...
|---|---|---|
| **Control Plane** (FastAPI) | Единственная точка входа для людей; AuthZ, реестр, оркестрация гейтов, CI-обработчик вебхуков, findings, аудит, карта покрытия; read-only витрины — реестры моделей/данных/решений/пользователей + карточки сущностей со сквозной связью актив⇄актор⇄находка (§5.2). | Кастомный слой даёт контроль над RBAC/видимостью, которого нет в готовых тулзах. |
...
| **scanners.py** (библиотека гейтов) | Общий код проверок (код / артефакты / зависимости / данные), импортируется **и CI-роутером, и эндпоинтами Control Plane** (одна логика в двух местах). | Build-time и интерактивные проверки не расходятся. |
...
- **Карточка сущности = единая история актива.** Клик по модели/датасету/актору открывает карточку: текущий статус, версии, lineage, открытые проблемы, решения гейта и таймлайн аудита (машинные коды событий — человекочитаемыми подписями).
- **Сквозная связность.** Триада **актив ⇄ актор ⇄ находка** кликабельна во всех панелях: из сработки — в карточку актива и причастного; из инспектора прогона (`/map`) — в карточки модели и датасета по lineage; владелец — в карточку актора. Один актив прослеживается из любой точки.
- **Эффективность реестра.** Живой текстовый поиск, сортировка по столбцам, счётчик «с проблемами» и владелец — чтобы «кто требует внимания» был виден сразу, без захода в каждую карточку.
- **Ничего не теряется.** Scoped-сканы качества данных (DATA-02/03/05) сканируют полезную нагрузку, а не зарегистрированный датасет; их можно привязать к карточке датасета, передав `dataset_id` в скан, а непривязанные показываются отдельным блоком на `/data`.

**Как это выглядит** (слева — тёмная тема, справа — светлая; клик открывает оригинал):

Реестр моделей — поиск, сортировка, KPI «с проблемами», владелец:

<table><tr>
<td><img src="_assets/img/04-registry.png" alt="Реестр моделей — тёмная"></td>
...
<table><tr>
<td><img src="_assets/img/05-model-card.png" alt="Карточка модели — тёмная"></td>
<td><img src="_assets/img/light/05-model-card.png" alt="Карточка модели — светлая"></td>
</tr></table>

Карточка датасета — богатая PII-схема с маскированием для ролей без допуска (DATA-04):

<table><tr>
<td><img src="_assets/img/12-dataset-schema.png" alt="Карточка датасета — тёмная"></td>
<td><img src="_assets/img/light/12-dataset-schema.png" alt="Карточка датасета — светлая"></td>
</tr></table>

Реестр решений — журнал аппрув-гейта (VIS-03) и принятий остаточного риска (GOV-02):

<table><tr>
<td><img src="_assets/img/08-decisions.png" alt="Реестр решений — тёмная"></td>
<td><img src="_assets/img/light/08-decisions.png" alt="Реестр решений — светлая"></td>
</tr></table>
...
| **AuditEvent** | актор, действие, объект, «имел ли право», время; **append-only** (БД-триггер LOG-02 — prevent) + hash-chain (MON-04 — detect) | сквозная |
