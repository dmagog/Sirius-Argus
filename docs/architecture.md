...
- **Честность данных.** Scoped-сканы качества данных (DATA-02/03/05) сканируют полезную нагрузку, а не зарегистрированный датасет; их можно привязать к карточке датасета, передав `dataset_id` в скан, а непривязанные показываются отдельным блоком на `/data` — чтобы находки не «терялись».
