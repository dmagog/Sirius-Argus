# Sirius — Архитектура безопасной MLOps-системы
...
Бизнес зарабатывает на ML ровно до первого инцидента. MLOps делает систему надёжной и воспроизводимой, но почти ничего не знает о безопасности. **Sirius** добавляет недостающий слой MLSecOps: видит весь пайплайн, понимает, что в проде, и управляет системой, а не латает дыры.
...
Возможности Sirius сгруппированы в 4 столпа (по HiddenLayer), каждый закрывает свой класс угроз:

| Столп | Что делает в Sirius | Тип защиты |
|---|---|---|
...
flowchart TB
    ...
    subgraph INT["Внутренняя docker-сеть — наружу НЕ публикуется"]
        CP["SIRIUS CONTROL PLANE<br/>FastAPI + Jinja/HTMX<br/>AuthN/Z · Реестр · Оркестрация гейтов<br/>CI(webhook) · Findings · Audit · Карта покрытия"]
...
| Этап ЖЦ | Активы (категории) | Ключевые угрозы | Контроли Sirius | Где в системе |
|---|---|---|---|---|
...
**B. Обучение.** DS обучает → `Run` (гиперпараметры, метрики, lineage) в MLflow, артефакт в MinIO → авто-генерится Model Card / security profile.
...
```mermaid
flowchart LR
    THR["Угроза<br/>@OWASP-ML / @ATLAS"] --> CTRL["Контроль Sirius"]
...
| OWASP ML Top 10 | Контроль Sirius |
|---|---|
