...
> Как строим: тонкими вертикальными срезами. Каждая итерация поднимается через `docker compose up` и **«зажигает» свой набор BDD-сценариев** зелёными. «Зелёный» = проходящий `pytest-bdd` против живой системы ([ADR-0004](adr/0004-bdd-methodology.md)), а не декларация. Любая итерация — валидная точка остановки.
>
