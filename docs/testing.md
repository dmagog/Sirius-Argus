...
- 50 BDD-сценариев ([bdd-catalog](threat-model/bdd-catalog.md)) написаны раньше кода; код итераций И0–И6 перевёл **49 из них в `green`** (58 pytest-функций, включая интеграционные serving/pipeline), остаётся 1 — `spec`/честный остаток (`SUP-06` ShadowLogic — предел статического анализа, закрывается HITL) ([risk-register](threat-model/risk-register.md) §«Статус контролей»).
...
|---|---|---|---|
| Приёмка (BDD) | поведения против живого стека (49 сценариев каталога green из 50; 58 pytest-функций) | pytest-bdd + httpx + проверки БД | да — сценарии раньше кода |
