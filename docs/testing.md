...
- 50 BDD-сценариев ([bdd-catalog](threat-model/bdd-catalog.md)) написаны раньше кода; код итераций И0–И6 перевёл **37 из них в `green`** (47 pytest-функций, включая интеграционные serving/pipeline), остальные 13 — `spec`/честный остаток ([risk-register](threat-model/risk-register.md) §«Статус контролей»).
...
|---|---|---|---|
| Приёмка (BDD) | поведения против живого стека (37 сценариев каталога green из 50; 47 pytest-функций) | pytest-bdd + httpx + проверки БД | да — сценарии раньше кода |
