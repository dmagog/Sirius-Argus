...
- 50 BDD-сценариев ([bdd-catalog](threat-model/bdd-catalog.md)) написаны раньше кода; код итераций И0–И6 перевёл **48 из них в `green`** (56 pytest-функций, включая интеграционные serving/pipeline), остальные 2 — `spec`/честный остаток (`SUP-06` ShadowLogic, `DOW-01` стоимостные квоты) ([risk-register](threat-model/risk-register.md) §«Статус контролей»).
...
|---|---|---|---|
| Приёмка (BDD) | поведения против живого стека (48 сценариев каталога green из 50; 56 pytest-функций) | pytest-bdd + httpx + проверки БД | да — сценарии раньше кода |
