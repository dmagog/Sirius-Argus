...
- 50 BDD-сценариев ([bdd-catalog](threat-model/bdd-catalog.md)) написаны раньше кода; код итераций И0–И6 перевёл **49 из них в `green`** (64 pytest-функции, включая интеграционные serving/pipeline и доп. контроли сверх каталога — Vault, risk-acceptance, оверсайз, сервис-аккаунт, evidence-HITL), остаётся 1 — `spec`/честный остаток (`SUP-06` ShadowLogic — предел статического анализа, закрывается HITL) ([risk-register](threat-model/risk-register.md) §«Статус контролей»).
...
|---|---|---|---|
| Приёмка (BDD) | поведения против живого стека (49 сценариев каталога green из 50; 64 pytest-функции) | pytest-bdd + httpx + проверки БД | да — сценарии раньше кода |
