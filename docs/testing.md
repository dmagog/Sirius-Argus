...
- 50 BDD-сценариев ([bdd-catalog](threat-model/bdd-catalog.md)) написаны раньше кода; код итераций И0–И6 перевёл **31 из них в `green`** (42 pytest-функции, включая интеграционные serving/pipeline), остальные 19 — `spec`/честный остаток ([risk-register](threat-model/risk-register.md) §«Статус контролей»).
...
|---|---|---|---|
| Приёмка (BDD) | поведения против живого стека (31 сценарий каталога green из 50; 42 pytest-функции) | pytest-bdd + httpx + проверки БД | да — сценарии раньше кода |
