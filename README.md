...
> Статус: в разработке, итерации [И0–И6](docs/roadmap.md). Сделано: **И0** (walking skeleton) + **ядро И1** (реестр, zero-trust RBAC, lineage/blast-radius, MON-02-гейт, реальный OIDC через Keycloak, Redis-шина событий, маскирование логов, реестр на обёрнутом MLflow) — 10 BDD-сценариев green. Документация идёт впереди кода намеренно — модель угроз и поведения (BDD) задают, что строим.
