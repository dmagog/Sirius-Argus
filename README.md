...
[![License](https://img.shields.io/badge/License-Elastic_2.0-005571?style=flat-square)](LICENSE)
[![Роспатент](https://img.shields.io/badge/Роспатент-св--во_№_2026685473-1D4ED8?style=flat-square)](#лицензия-и-правовой-статус)

**Sirius Argus — платформа MLSecOps с открытым исходным кодом (source-available): безопасность и управляемость на всём жизненном цикле ML-модели, в едином контуре.** Одна контрольная панель объединяет разрозненные инструменты безопасности и управления — и только через неё модель попадает в эксплуатацию: со сканами, подписью, проверкой по политикам, ручным одобрением для критичных версий и неизменяемым аудитом. По каждому активу видно, какие угрозы закрыты и каким контролем. Разворачивается в своём периметре: данные, артефакты и секреты наружу не уходят.
...
- **Реальные тулзы.** modelaudit, OpenSSF model-signing, Semgrep, detect-secrets, pip-audit — поверх собственных сканеров; секреты — в хранилище с Vault-совместимым API (OpenBao / HashiCorp Vault).
- **Интеграционная платформа, а не обёртка.** Обязательное ядро — только control-plane, PostgreSQL и Redis. Остальное — IdP, хранилище, тренер, реестр моделей, наблюдаемость — подключается по стандартным протоколам (OIDC, S3, вебхуки), и любой компонент можно заменить совместимым. Само ядро тоже на стандартах: PostgreSQL и Redis совместимы с реестровыми российскими аналогами (Postgres Pro, Valkey). [Где подключить своё →](docs/integration.md)
...
- **Лицензия: [Elastic License 2.0](LICENSE)** (source-available). Разрешено бесплатно использовать, копировать, модифицировать и распространять платформу, в том числе в коммерческих целях внутри организации; раскрывать свои доработки не требуется. Не допускается предоставлять платформу третьим лицам как управляемый сервис (managed service) и удалять лицензионные уведомления. Коммерческая лицензия на иных условиях — по договору с правообладателем.
- **Правообладатель:** Мамарин Георгий Алексеевич (Российская Федерация).
- **Государственная регистрация программы для ЭВМ** (ст. 1262 ГК РФ): свидетельство Роспатента **№ 2026685473 от 21.08.2026** — «Программная платформа обеспечения безопасности процессов машинного обучения (MLSecOps) „Sirius Argus"».
- **Контакт:** georgy.mamarin@gmail.com
