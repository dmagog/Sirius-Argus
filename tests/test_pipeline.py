...
def up():
    assert httpx.get(f"{BASE}/health", timeout=10).status_code == 200, "сначала `DEV_AUTH=1 make up`"
    assert httpx.get(f"{SERVING}/health", timeout=10).status_code == 200, "сервинг не поднят"


@when("модель проходит полный жизненный цикл")
def lifecycle():
    dv = httpx.post(f"{BASE}/api/datasets", headers=tok("DE"),
                    json={"name": "loans", "sensitivity": "open", "source": "internal://curated/loans"}, timeout=10).json()["dataset_version_id"]
    mid = httpx.post(f"{BASE}/api/models", headers=tok("DS"),
                     json={"name": "loan-scoring", "type": "boosting", "criticality": "financial"}, timeout=10).json()["model_id"]
    # 1) вредоносный приём → блок
    S["mal"] = httpx.post(f"{BASE}/api/models/{mid}/ingest", headers=tok("DS"), content=pickle.dumps(_Evil()), timeout=15).status_code
